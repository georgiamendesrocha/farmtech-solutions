...
get_weather_data <- function(latitude, longitude, city_name = "Local selecionado") {
  ...
  response <- tryCatch({
    GET(base_url, query = params, timeout(15))
  }, error = function(e) {
  ...
  cat(sprintf("  ☁️  Condição:            %s\n", get_weather_description(current$weather_code)))

  print_header("Previsão para os Próximos 7 Dias")
  daily <- data$daily

  for (i in seq_len(min(7, length(daily$time)))) {
    date <- daily$time[i]
    ...
    cat(sprintf("\n  %s\n", date))
    cat(sprintf("     Temp: %.1f°C a %.1f°C | Chuva: %.1f mm | Vento: %.1f km/h\n",
  ...
  if (total_rain > 50) {
    ...
    cat("  • Volume de chuva moderado - condições razoáveis para o manejo.\n")
  }

  if (avg_temp > 30) {
    cat("  • Temperaturas elevadas - monitore estresse hídrico e horários de aplicação.\n")
  } else if (avg_temp < 15) {
    cat("  • Temperaturas baixas - atenção com culturas sensíveis ao frio.\n")
  } else {
    cat("  • Temperaturas adequadas para a maioria das culturas.\n")
  }

  if (max_wind > 40) {
    cat("  • Ventos fortes previstos - evite pulverizações em horários críticos.\n")
  }
...
main <- function() {
  ...
  cat("1. São Paulo, SP\n")
  cat("2. Cuiabá, MT (região forte para soja)\n")
  cat("3. Patrocínio, MG (região forte para café)\n")
  cat("4. Londrina, PR\n")
  ...
  if (choice %in% names(locations)) {
    ...
    cat("\n")
    lat <- suppressWarnings(as.numeric(readline(prompt = "Digite a latitude: ")))
    lon <- suppressWarnings(as.numeric(readline(prompt = "Digite a longitude: ")))
  ...
  cat("\n")
  print_divider()
  cat("Dados fornecidos por: Open-Meteo API\n")
  cat("https://open-meteo.com\n")
  print_divider()
  cat("\n")
}
